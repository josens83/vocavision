/**
 * TEPS L1 (중급 300-400점) Words Seed Script
 *
 * Run with: npx tsx prisma/seed-teps-l1.ts
 */
...
async function main() {
  console.log('Starting TEPS L1 word seed...');
  ...
  const TEPS_L1_WORDS = jsonData.levels.L1.words;

  console.log(`Total words to process: ${TEPS_L1_WORDS.length}`);
  ...
  for (const wordText of TEPS_L1_WORDS) {
  ...
  console.log(`\n=== Seed Complete ===`);
  console.log(`Total TEPS-L1 words in database: ${finalCount}`);
}
