/**
 * TEPS L2 (중상급 400-500점) Words Seed Script
 *
 * Run with: npx tsx prisma/seed-teps-l2.ts
 */
...
async function main() {
  console.log('Starting TEPS L2 word seed...');
  ...
  const TEPS_L2_WORDS = jsonData.levels.L2.words;

  console.log(`Total words to process: ${TEPS_L2_WORDS.length}`);
  ...
  for (const wordText of TEPS_L2_WORDS) {
  ...
  console.log(`\n=== Seed Complete ===`);
  console.log(`Total TEPS-L2 words in database: ${finalCount}`);
}
