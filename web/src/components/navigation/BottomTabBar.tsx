...
const icons = {
  home: (active: boolean) => (
    <svg className={`w-6 h-6 ${active ? 'text-teal-500' : 'text-gray-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              ...
              <span className={`text-[10px] mt-1 truncate ${active ? 'text-teal-500 font-medium' : 'text-gray-400'}`}>
                {tab.label}
              </span>
            </Link>
          );
